...
class KPIService {
    ...
    async recommendWeights() {
        ...
        // (RideRepository import가 있어야 실행 가능)
        ...
        // 데이터가 너무 적으면 분석 불가 (예: 최소 사고 데이터 5건, 정상 10건)
        const isNotEnoughData = totalAccidents < 5 || totalNormals < 10;

        // 2. KPI별 발생 통계 조회 (KPIRepository에 추가된 메서드 호출)
        ...
        const recommendations = kpiStats.map((stat) => {
            ...
            if (freqInNormal > 0) {
                ...
                riskRatio = 5.0; // 정상 주행에선 안 나타나는데 사고 때만 나타나면 매우 위험
            } else {
                riskRatio = 1.0; // 둘 다 안 나타나면 중립
            }

            // [가중치 추천 공식]
            // 현재 가중치 * (위험도 계수 ^ 0.5) -> 급격한 변화 방지
            ...
            else if (suggested < currentWeight * 0.9)
                reason = `사고 연관성 낮음`;
            else
                reason = "적정 수준 유지";
